...
func testSpiderAlgorithms(net *lntest.NetworkHarness, t *harnessTest, algo int32) {
	...
	// Note that vanilla LND does now allow zero fee. Modification has been done to LND
	...
	printStats := func() {
		...
		// aggregate per-intent pair success rates
				...
				case <-paymentTick[i]:
					...
					paymentTimeoutWrapper := func(i int) {
						...
						case s := <-sendok:
							...
							case 4:
								mux.Lock()
								fmt.Printf("%v->%v: lp full queue buffer\n", nodeNames[payIntents[i][0]], nodeNames[payIntents[i][1]])
								mux.Unlock()
							}
						case <-timeout:
							mux.Lock()
